...
const TourManagerPage = () => {
  ...
  const ballNumbers = useMemo(() => {
    const mode = tourManagerGameMode || "9-ball";
  ...
  // Load logo config from Firestore (config/tour-manager)
  useEffect(() => {
    const loadLogoConfig = async () => {
      try {
        const configRef = doc(db, "config", TOUR_MANAGER_CONFIG_ID);
  ...
  // Fetch logos from Firestore (same list as Players tab)
  useEffect(() => {
  ...
  const handleSelectLogo = (slot: 1 | 2, logo: Logo) => {
    ...
    const configRef = doc(db, "config", TOUR_MANAGER_CONFIG_ID);
    setDoc(configRef, slot === 1 ? { logo1URL: url } : { logo2URL: url }, { merge: true }).catch(console.error);
  };
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [player1Score, player2Score, currentTurn, loading, player1, player2, pocketedBalls, tourManagerGameMode, raceTo]);
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      if (e.key >= "0" && e.key <= "9") {
        ...
        const currentBallNumbers = getBallNumbers(tourManagerGameMode);
    ...
    <div className="p-2 sm:p-4 md:p-6 h-screen flex flex-col bg-transparent overflow-hidden">
      <div className="mx-auto flex-1 flex flex-col relative w-full" style={{ maxWidth: "1920px" }}>
        ...
        <LogoSelectionModal
          ...
          onSelect={(logo) => handleSelectLogo(1, logo)}
          title="Select Logo 1"
        />
        <LogoSelectionModal
          ...
          onSelect={(logo) => handleSelectLogo(2, logo)}
          title="Select Logo 2"
        />
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-wrap justify-center">
            <div className="flex space-x-1 sm:space-x-2 md:space-x-3 lg:space-x-4 bg-amber-50 rounded-full px-2 sm:px-4 md:px-6 py-1 flex-wrap justify-center">
              {ballNumbers.length > 0 ? (
                ...
                <div className="text-xs text-gray-500 px-2">No balls for {tourManagerGameMode || "current"} mode</div>
              )}
            </div>
