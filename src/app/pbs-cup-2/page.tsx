...
const PBS_CUP_2_CONFIG_ID = "pbs_cup_2";
...
const LiveGenericPage = () => {
  ...
  const { pbsCup2IsLive: isLive, setPbsCup2IsLive: setIsLive, pbsCup2GameMode: gameMode, setPbsCup2GameMode: setGameMode } = useLive();
  ...
  useEffect(() => {
    const loadLogoConfig = async () => {
      try {
        const configRef = doc(db, "config", PBS_CUP_2_CONFIG_ID);
  ...
  const handleSelectLogo = (slot: 1 | 2, logo: Logo) => {
    ...
    setDoc(doc(db, "config", PBS_CUP_2_CONFIG_ID), slot === 1 ? { logo1URL: url } : { logo2URL: url }, { merge: true }).catch(console.error);
  };
  ...
  useEffect(() => {
    const loadMatchData = async () => {
      try {
        const matchDocRef = doc(db, "current_match", "pbs_cup_2");
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6 h-screen flex flex-col bg-transparent overflow-hidden">
      <div className="mx-auto flex-1 flex flex-col relative w-full" style={{ maxWidth: "1920px" }}>
        ...
        <PbsCup2EightSingleOverlay players={players} canEdit={canSelectPlayers} />

        {/* Players Scoring Container - Bottom - 75% width on desktop; 2x photos, overflow visible */}
        <div className="mt-auto w-full md:w-[75%] max-w-full mx-auto flex items-center justify-center px-2 sm:px-4 md:px-0 overflow-visible">
          {/* Player 1 Section - Dark Indigo Background */}
          <div className="bg-indigo-950 flex items-center h-10 sm:h-12 md:h-14 overflow-visible">
            ...
            <button
              ...
              className={`w-[80px] h-[80px] sm:w-[100px] sm:h-[100px] md:w-[120px] md:h-[120px] lg:w-[140px] lg:h-[140px] rounded-full overflow-hidden shrink-0 transition-all duration-300 flex items-center justify-center mx-1 sm:mx-1.5 md:mx-2 border-2 border-white ${
                canSelectPlayers
            ...
            <button
              ...
              className={`px-2 sm:px-4 md:px-6 lg:px-10 h-full flex items-center ${
                canSelectPlayers
          ...
          {/* Scores and Race To - Dark Indigo Center */}
          <div
            className="flex items-center justify-center space-x-2 sm:space-x-3 md:space-x-4 lg:space-x-5 bg-indigo-950 h-10 sm:h-12 md:h-14 min-w-[120px] sm:min-w-[160px] md:min-w-[200px] lg:min-w-[240px] px-2 sm:px-3 md:px-4"
          >
          ...
          {/* Player 2 Section - Dark Indigo Background */}
          <div className="bg-indigo-950 flex items-center h-10 sm:h-12 md:h-14 overflow-visible">
