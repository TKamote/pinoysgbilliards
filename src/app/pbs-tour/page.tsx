...
const PBS_TOUR_CONFIG_ID = "pbs-tour";
...
const PbsTourPage = () => {
  ...
  const { pbsTourIsLive, setPbsTourIsLive, pbsTourGameMode, setPbsTourGameMode } =
    useLive();
  ...
  const ballNumbers = useMemo(() => {
    const mode = pbsTourGameMode || "9-ball";
  ...
  useEffect(() => {
    const loadLogoConfig = async () => {
      try {
        const configRef = doc(db, "config", PBS_TOUR_CONFIG_ID);
  ...
  const handleSelectLogo = (logo: Logo) => {
    ...
    setLogo1URL(url);
    setDoc(doc(db, "config", PBS_TOUR_CONFIG_ID), { logo1URL: url }, { merge: true }).catch(console.error);
  };
  ...
  useEffect(() => {
    const loadMatchData = async () => {
      try {
        const matchDocRef = doc(db, "current_match", "pbs-tour");
        ...
        if (matchDoc.exists()) {
          ...
          if (
            ...
            setPbsTourGameMode(matchData.gameMode as GameMode);
          }
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [player1Score, player2Score, currentTurn, loading, player1, player2, pocketedBalls, pbsTourGameMode, raceTo]);
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      if (e.key >= "0" && e.key <= "9") {
        ...
        const currentBallNumbers = getBallNumbers(pbsTourGameMode);
  ...
  }, [isManager, pbsTourIsLive, pbsTourGameMode, handleResetBalls, loading, showWinnerModal, handleWinnerModalClose]);
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6 h-screen flex flex-col bg-transparent overflow-hidden">
      <div className="mx-auto flex-1 flex flex-col relative w-full" style={{ maxWidth: "1920px" }}>
        ...
        {/* Players Scoring Container - Bottom - 70% width on desktop for YouTube Shorts margin; overflow-visible for enlarged photo experiment */}
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-wrap justify-center">
            <div className="flex space-x-1 sm:space-x-2 md:space-x-3 lg:space-x-4 bg-amber-50 rounded-full px-2 sm:px-4 md:px-6 py-1 flex-wrap justify-center">
              {ballNumbers.length > 0 ? (
                ...
                <div className="text-xs text-gray-500 px-2">No balls for {pbsTourGameMode || "current"} mode</div>
              )}
            </div>
