    ...
    <div className="p-2 sm:p-4 md:p-6 min-h-screen md:h-screen flex flex-col bg-transparent overflow-y-auto md:overflow-hidden">
      <div className="mx-auto flex-1 flex flex-col relative w-full" style={{ maxWidth: "1920px" }}>
        ...
        <div className="md:hidden flex flex-col gap-3 w-full pb-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex gap-2">
              <button
                ...
                className={`shrink-0 rounded-full overflow-hidden border-2 border-white bg-white shadow w-12 h-12 flex items-center justify-center ${canSelectPlayers ? "active:opacity-90" : "cursor-default"}`}
              >
            ...
            <button
              ...
              className={`text-white px-4 py-2 rounded-full font-bold text-sm shrink-0 ${
                isLive ? "bg-linear-to-r from-red-600 to-red-800 animate-pulse" : "bg-gray-500 active:bg-gray-600"
              }`}
            >
              {isLive ? "LIVE" : "GO LIVE"}
            </button>
          ...
          <div className="flex justify-center">
            {showRaceToInput && isManager ? (
              <input
                ...
                className="w-16 text-center text-base font-bold text-gray-900 border-2 border-gray-400 rounded-lg px-2 py-1"
                min={1}
                max={50}
                autoFocus
              />
          ...
          {/* P1 — red */}
          <div className="bg-red-700 rounded-xl p-3 flex items-center gap-3 min-w-0">
            <button
              type="button"
              onClick={() => canSelectPlayers && setShowPlayer1Modal(true)}
              className={`w-16 h-16 shrink-0 rounded-full overflow-hidden border-2 border-white ${getPlayer1Photo() ? "bg-transparent" : "bg-red-600"}`}
            >
              {getPlayer1Photo() ? (
            ...
            <div className="flex-1 min-w-0">
              <button type="button" onClick={() => canSelectPlayers && setShowPlayer1Modal(true)} className="text-left w-full">
                <span className="text-sm font-semibold text-white leading-snug line-clamp-2">{getPlayer1Name()}</span>
              </button>
              <div className="flex items-center gap-2 mt-1">
                <button type="button" className="text-xs text-white/90 px-2 py-0.5 border border-white/50 rounded" onClick={() => setPlayer1Score((s) => Math.max(0, s - 1))} aria-label="P1 minus">−</button>
                <span className="text-xl font-bold text-white tabular-nums">{player1Score}</span>
                <button type="button" className="text-xs text-white/90 px-2 py-0.5 border border-white/50 rounded" onClick={() => setPlayer1Score((s) => s + 1)} aria-label="P1 plus">+</button>
              </div>
            </div>
          ...
          {/* P2 — blue */}
          <div className="bg-blue-700 rounded-xl p-3 flex items-center gap-3 min-w-0">
            ...
            <div className="flex-1 min-w-0 flex flex-col items-end text-right">
              <button type="button" onClick={() => canSelectPlayers && setShowPlayer2Modal(true)} className="text-right w-full">
                <span className="text-sm font-semibold text-white leading-snug line-clamp-2">{getPlayer2Name()}</span>
              </button>
              <div className="flex items-center gap-2 mt-1 justify-end">
                <button type="button" className="text-xs text-white/90 px-2 py-0.5 border border-white/50 rounded" onClick={() => setPlayer2Score((s) => Math.max(0, s - 1))} aria-label="P2 minus">−</button>
                <span className="text-xl font-bold text-white tabular-nums">{player2Score}</span>
                <button type="button" className="text-xs text-white/90 px-2 py-0.5 border border-white/50 rounded" onClick={() => setPlayer2Score((s) => s + 1)} aria-label="P2 plus">+</button>
              </div>
            </div>
            <button
              type="button"
              onClick={() => canSelectPlayers && setShowPlayer2Modal(true)}
              className={`w-16 h-16 shrink-0 rounded-full overflow-hidden border-2 border-white ${getPlayer2Photo() ? "bg-transparent" : "bg-blue-600"}`}
            >
              {getPlayer2Photo() ? (
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 bg-amber-50 rounded-2xl px-3 py-3">
            {ballNumbers.map((ballNum) => {
              ...
              return (
                ...
                >
                  <Image src={`/ballicons/ball-${ballNum}.png`} alt={`Ball ${ballNum}`} width={40} height={40} className="object-contain w-full h-full" />
                </button>
              );
            })}
